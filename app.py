...
.signal-card { background: #0d1117; border: 1px solid #21262d; border-radius: 12px; padding: 16px; margin-bottom: 12px; }
...
def fetch_token_screener(chains, api_key):
...
with st.sidebar:
    ...
    st.markdown('<div class="info-box">Your keys are used only for this session and are never stored or shared.</div>', unsafe_allow_html=True)
    nansen_key = st.text_input("Nansen API Key", type="password", placeholder="Paste your Nansen API key...", help="Get yours at app.nansen.ai")
    st.markdown("---")
    st.markdown("### 📬 Telegram Alerts *(optional)*")
    tg_token = st.text_input("Telegram Bot Token", type="password", placeholder="1234567890:AAF...")
    tg_chat_id = st.text_input("Telegram Chat ID", placeholder="123456789")
    st.markdown("---")
    st.markdown("### 🔗 Chains")
...
if "results" in st.session_state and st.session_state.results:
    ...
    st.markdown("""<div style="text-align:center;padding:60px 20px"><div style="font-size:3rem">🧠</div>
    <div style="font-family:'Space Mono',monospace;font-size:1.1rem;color:#555;margin-top:12px">
    Enter your credentials in the sidebar<br>and hit <span style="color:#00ff88">Run Scan Now</span></div></div>
